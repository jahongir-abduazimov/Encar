...
export async function generateMetadata({ params }: { params: { id: string } }): Promise<Metadata> {
  try {
    ...
    const description = `${car.name} ${car.year} года выпуска, ${russianBodyType}. ${russianFuelType} двигатель, ${russianTransmission} коробка передач, пробег ${car.miliage.toLocaleString('ru-RU')} км. Цена: ${car.price.toLocaleString('ru-RU')} ${car.car_pricing?.currency || 'USD'}. Купить ${car.name.toLowerCase()} в России.`
    ...
    const openGraphDescription = `${car.name} ${car.year} года, ${russianBodyType}. ${russianFuelType} двигатель, ${car.miliage.toLocaleString('ru-RU')} км пробега. Цена ${car.price.toLocaleString('ru-RU')} ${car.car_pricing?.currency || 'USD'}.`
    ...
    return {
      ...
      openGraph: {
        ...
        type: 'website',
        url: `https://encar.com/cars/${params.id}`,
        siteName: 'EnCar - Автомобили в России',
        locale: 'ru_RU',
        images: car.car_medias.length > 0 ? [
          {
      ...
      alternates: {
        canonical: `https://encar.com/cars/${params.id}`,
        languages: {
          'ru-RU': `https://encar.com/ru/cars/${params.id}`,
        },
      },
      other: {
        'geo.region': 'RU',
    ...
    return {
      ...
      description: 'Подробная информация об автомобиле на EnCar. Купить автомобиль в России.',
...
function generateStructuredData(car: CarDetailType) {
  ...
  return {
    ...
    image: car.car_medias.length > 0 ? car.car_medias.map(media => media.media) : [],
    description: `${car.name} ${car.year} года выпуска, ${russianBodyType}. ${russianFuelType} двигатель ${car.engine_capacity}л, ${russianTransmission} коробка передач. Пробег: ${car.miliage.toLocaleString('ru-RU')} км. Цвет: ${car.color.name}.`,
    additionalProperty: [
      {
        '@type': 'PropertyValue',
        name: 'Год выпуска',
    ...
    itemListElement: [
      {
        ...
        position: 1,
        name: 'Главная',
        item: 'https://encar.com'
      },
      {
        '@type': 'ListItem',
        position: 2,
        name: 'Автомобили',
        item: 'https://encar.com/cars'
      },
      {
        '@type': 'ListItem',
        position: 3,
        name: russianBodyType,
        item: `https://encar.com/cars?body_type=${car.body_type.name}`
      },
      {
        '@type': 'ListItem',
        position: 4,
        name: `${car.name} ${car.year}`,
        item: `https://encar.com/cars/${car.id}`
      }
    ]
  }
}
